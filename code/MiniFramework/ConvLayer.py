...
class ConvLayer(CLayer):
    ...
    def backward_col2img(self, delta_in, layer_idx):
        ...
        delta_out = col2img(col_delta_out, self.x.shape, self.FH, self.FW, self.stride, self.padding, self.OutH,
                            self.OutW)
    ...
    def forward_numba(self, x, train=True):
        ...
        if self.padding > 0:
            self.padded = np.pad(self.x, ((0, 0), (0, 0), (self.padding, self.padding), (self.padding, self.padding)),
                                 'constant')
    ...
    def backward_numba(self, delta_in, flag):
        ...
        dz_stride_1 = expand_delta_map(delta_in, self.batch_size, self.OutC, self.InH, self.InW, self.OutH, self.OutW,
                                       self.FH, self.FW, self.padding, self.stride)
